/*Cross-site scripting: El atacante logra meter codigo malicioso dentro de una agina web
un usuario normal escribiria: Me gusta este producto
un atacante escribiria: <script>alert("Hackeado")</script
El navegador cree que el codigo es parte de la pagina y lo ejecuta
*/
...
pattern --> Define reglas 
pattern.test(value) --> Verifica si cumple la regla
new TextEncoder().encode() --> Convierte texto a bytes
await crypto.subtle.digest("SHA-256", data) --> Aplica el algoritmo SHA-256
Array.from(new Uint8Array(buffer)) --> Convierte datos binarios en array 
toString(16) --> Convierto a hexadecimal 
padStart(2, "0") --> Asegura 2 digitos x cada byte
join --> une una lista en un string
localStoage --> Guarda datos en el navegador 
JSON.parse() --> Convierte texto a objeto 
localStrorage.setItem() --> Guarda datos 
JSON.stringify() --> Convierte objeto a texto
getUsers() --> obtiene usuarios desde localStorage
users.some() --> Verifica si existe usuario 
crypto.randomUUID() --> Genera un ID único
toISOString() --> Convierte fecha a formato ESTANDAR
...
function stripTags(str) {
  if (typeof str !== "string") return "";
  return str.replace(/<[^>]*>/g, ""); //Elimina etiquetas html 
}

// Sanitiza texto visible (nombre, email). NO usar en contraseñas.
//Esta funcion ocupa 2 metodos una que reemplaza los caracteres peligrosos y otra que  si no string reemplaza el codigo por nada. Sanitiza el texto para evitar ataques xss
function sanitize(value) {
  return escapeHTML(stripTags(String(value).trim()));//función de programación que se utiliza para eliminar todas las etiquetas HTML y PHP de una cadena de texto, convierte a str y quita espacios 
}
...
const RULES = { //define reglas de validación
  nombre: {
    minLen: 2,
    maxLen: 50,
...
function validateField(fieldName, value) { //busca por clave(ej: nombre) y luego busca los valores de esa clave (lo que puso el usuario)
  ...
  if (!value || value.length === 0)         return { valid: false, error: rule.messages.required }; //¿Es identico a cero, es decir, vacio?
...
const STORAGE_KEY = "deluxe_users"; //Constante de almacenamiento con clave 
...
async function hashPassword(password) {
  ...
  const buffer  = await crypto.subtle.digest("SHA-256", data); //Aplica el algoritmo SHA-256, hashea
  return Array.from(new Uint8Array(buffer)) //Convierte en array
...
function showFormMessage(form, message, isError = false) {
  ...
  el.textContent       = message;             // textContent — nunca innerHTML, muestra el mensaje 
...
function initLoginForm() {
  ...
  form.addEventListener("submit", async e => { //Detecta cuando se envia el formulrio
