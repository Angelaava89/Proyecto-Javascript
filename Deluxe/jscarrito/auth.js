...
function sanitize(value) {
  return escapeHTML(stripTags(String(value).trim()));
}
...
// localStorage : Esta propiedad te da acceso a un objeto de almacenamiento web que te permite guardar datos en el navegador de forma persistente
...
function loginUser(email, passwordHash) { //Agrega el email y la password 
  ...
  sessionStorage.setItem( //Guarda la sesión del usuario en sessionStorage
    "deluxe_session",
    JSON.stringify({ id: user.id, nombre: user.nombre, email: user.email })//Json trabaja con cadena de texto plano, por eso no esta la contraseña 
  );
