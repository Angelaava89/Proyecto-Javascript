...
cantidadInputs.forEach((input,index)=>{
    input.addEventListener("change",(e)=>{
        ...
        if(e.target.classList.contains("cantidad")){
            contenedorCarrito[index].Cantidad=parseInt(e.target.value)|| 1 //se va traspasar la cantidad al index del carrito
            ActualizarTotalCompra()
            ContadorDeArticulos()
        }
    })
    ...
    BotonEliminar.forEach(item=>{
        item.addEventListener("click", (e)=>{
            if(e.target.classList.contains("quitar")){
                const index= parseInt(e.target.getAttribute("data-index"))//transforma  el index en un numero entero
